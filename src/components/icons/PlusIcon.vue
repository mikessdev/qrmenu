<script setup lang="ts">
defineProps({
  color: {
    type: String,
    default: '#fff'
  },
  width: {
    type: Number,
    default: 48
  },
  height: {
    type: Number,
    default: 48
  },
  variante: {
    type: String,
    default: '01'
  }
});
</script>
<template>
  <svg
    v-if="variante === '01'"
    xmlns="http://www.w3.org/2000/svg"
    :height="height"
    viewBox="0 -960 960 960"
    :width="width"
  >
    <path
      d="M453-280h60v-166h167v-60H513v-174h-60v174H280v60h173v166Zm27.266 200q-82.734 0-155.5-31.5t-127.266-86q-54.5-54.5-86-127.341Q80-397.681 80-480.5q0-82.819 31.5-155.659Q143-709 197.5-763t127.341-85.5Q397.681-880 480.5-880q82.819 0 155.659 31.5Q709-817 763-763t85.5 127Q880-563 880-480.266q0 82.734-31.5 155.5T763-197.684q-54 54.316-127 86Q563-80 480.266-80Zm.234-60Q622-140 721-239.5t99-241Q820-622 721.188-721 622.375-820 480-820q-141 0-240.5 98.812Q140-622.375 140-480q0 141 99.5 240.5t241 99.5Zm-.5-340Z"
      :fill="color"
    />
  </svg>
  <svg
    v-if="variante === '02'"
    :width="width"
    :height="height"
    viewBox="0 0 20 20"
    fill="none"
    xmlns="http://www.w3.org/2000/svg"
  >
    <path
      d="M19.375 7.5H12.5V0.625C12.5 0.28 12.22 0 11.875 0H8.125C7.78 0 7.5 0.28 7.5 0.625V7.5H0.625C0.28 7.5 0 7.78 0 8.125V11.875C0 12.22 0.28 12.5 0.625 12.5H7.5V19.375C7.5 19.72 7.78 20 8.125 20H11.875C12.22 20 12.5 19.72 12.5 19.375V12.5H19.375C19.72 12.5 20 12.22 20 11.875V8.125C20 7.78 19.72 7.5 19.375 7.5Z"
      :fill="color"
    />
  </svg>
</template>
